...
class GetReceiptContours:
    ...
    def approximate_contours(self):
        ...
        for i, cnt in enumerate(self.contours):
            ...
            if arclen != 0 and self.img_size * 0.4 < area < self.img_size * 0.95:
    ...
    def limited_to_rectangles(self):
        ...
        for contour in self.approx_contours:
                if 4 <= len(contour) <= 7:  # 頂点が4点~７点の輪郭のみにする
                    max_abs_cos = get_max_abs_cosine(contour)
                    if max_abs_cos < math.cos(math.radians(70)):  # なす角が70°~110°のみにする
...
def cut_out_receipts_main(file_):
    input_path_list = file_
    input_path = input_path_list  # 現状、読み取り対象の画像は1枚しか対応していないため
    print("処理中...")
